        ...
        <div class="row justify-content-center">
            <div class="col-md-6 col-lg-5">
                <div class="card checkout-card p-4 p-md-5">
                    ...
                    <!-- Form action alterado para o próprio carrinho.php -->
                    <form action="carrinho.php" method="POST">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h6 class="fw-bold mb-0">Resumo do Pedido</h6>
                            <a href="carrinho.php?limpar=1" class="text-danger small text-decoration-none">Esvaziar</a>
                        </div>
                        <?php foreach ($pratos_carrinho as $item): ?>
                        <div class="d-flex justify-content-between mb-2 small">
                            <span class="text-muted"><?= htmlspecialchars($item['nome']) ?> (x<?= $item['qtd'] ?>)</span>
                            <span class="fw-bold">R$ <?= number_format($item['subtotal'], 2, ',', '.') ?></span>
                        </div>
                        <?php endforeach; ?>
                        
                        <div class="d-flex justify-content-between align-items-center mb-4 mt-3 pt-3 border-top">
                            <span class="h6 fw-bold mb-0">TOTAL</span>
                            <span class="total-price">R$ <?= number_format($total_carrinho, 2, ',', '.') ?></span>
                        </div>
                        <button type="submit" class="btn btn-confirm w-100 shadow-sm mb-3">Finalizar Compra</button>
